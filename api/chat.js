...
${ubicacion ? `Ubicación GPS: ${String(ubicacion).slice(0, 250)}` : ""}`.trim();
    ...
    if (geminiApiKey) {
      ...
      for (const model of geminiModels) {
        try {
          ...
          errores.push(`Gemini (${model}) falló: HTTP ${resGemini.status}`);
        } catch (error) {
    ...
    if (openRouterApiKey) {
      ...
      for (const model of openRouterModels) {
        try {
          ...
          errores.push(`OpenRouter (${model}) falló: HTTP ${resOR.status}`);
        } catch (error) {
    ...
    // ==========================================
    // SI TODO FALLA (CORTAMOS WIKIPEDIA)
    // ==========================================
    return res.status(503).json({
      error: "Sistemas ocupados",
      detalle: "Mis cerebros (Gemini y OpenRouter) están saturados en este momento. Dame un par de minutos y volvé a preguntarme.",
      logs: errores
    });
